...
- 目標作成の直後に「コミット」画面（復元用メール1つ＋カード登録。パスワードなし）
  - カードが登録できるまで目標は作られない（サーバー側 `create-goal` も `card_required` で断る）
  - すでに使われているメールなら、その場でコードを送ってログインし前の記録とカードを引き継ぐ
- カード登録（Stripe PaymentSheet・登録時は0円）
- 機種変更時の復元（メールに届く6桁コードでログイン。パスワードでのログインも残している）
- 週次の自動判定と、未達週のみの自動課金（Supabase Edge Function）
- 週次実行の自動化（pg_cron。`0004_judge_weeks_cron.sql`）
- 法務ページ3種（利用規約・プライバシーポリシー・特定商取引法に基づく表記）
- アカウント削除機能（App Store ガイドライン5.1.1(v) の必須要件）
- プレミアム会員（モックのトグル）の廃止

### 動作確認済み（2026-09-07・Stripeテストモード）

カード登録 → 週次判定 → 自動課金 → Webhook確定 の一連が本番環境で通ることを確認した。
`weeks` の `end_date` を過去日にして judge-weeks を叩き、
`missed-charge-succeeded` と ¥300 の課金が発生することを確認している。
...
**開示を求められたら必ず応じてください。** 応じないと、省略していること自体が
特定商取引法違反になります。

`kakugo.support@gmail.com` は、必ず普段使う受信箱へ転送設定しておくこと。
返金・解約の問い合わせが届く窓口で、見落とすと消費者庁案件になり得ます。
...
- **Email → Magic Link のメールテンプレート**に `{{ .Token }}` を入れる
  （アプリは6桁コードでログインするため。例: 「ログインコード: {{ .Token }}」）
- 「メール確認」を有効にしている場合、復元用メールは確認リンクを開くまで `new_email` に留まる。
  その間もカード登録・課金は動く（確認前はそのメールでの復元だけができない）
...
| カテゴリ | 教育 |
| 年齢制限 | 17+ を推奨（課金を伴うため） |
| スクリーンショット | 6.5インチ枠。`1242×2688` / `2688×1242` / `1284×2778` / `2778×1284` のいずれか（App Store Connect が受け付けるのはこの実寸のみ）。手持ちのiPhoneの解像度が違う場合は、撮ったあとにリサイズする |
